(() => {
  ...
  function createMouse() {
    ...
    el.className =
      "mouse " +
      (type < 0.12 ? "gold" : type < 0.24 ? "black" : "");
  ...
  function updateHud() {
    ...
    livesEl.textContent = [full, empty].filter(Boolean).join(" ");
  ...
  function loop(time) {
    ...
    const interval = Math.max(0.32, 0.92 - score * 0.009);

    if (spawn > interval) {
      spawn = 0;
      createMouse();

      if (endlessMode && score > 8 && Math.random() < Math.min(0.5, 0.12 + score * 0.004)) {
        createMouse();
      } else if (!endlessMode && score > 18 && Math.random() < 0.22) {
        createMouse();
      }
    }

    for (const mouse of [...mice]) {
      mouse.phase += dt * (5 + score * 0.02);

      if (mouse.turn && Math.sin(mouse.phase * 0.7) > 0.995) {
        mouse.vx *= -1;
      }
